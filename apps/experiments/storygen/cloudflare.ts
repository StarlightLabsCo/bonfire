...
export async function uploadImageToR2(imageURL: string, id: string) {
  ...
  return `${process.env.S3_PUBLIC_URL}/${id}.png`;
}

export async function uploadPcmToR2(pcmBuffer: Buffer, id: string) {
  const uploadCommand = new PutObjectCommand({
    Bucket: process.env.S3_ASSETS_BUCKET,
    Key: id + '.pcm',
    Body: pcmBuffer,
  ...
  return `${process.env.S3_PUBLIC_URL}/${id}.pcm`;
}
