...
const CarouselContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  ...
  return (
    <div ref={carouselRef} className="flex flex-col overflow-hidden grow">
      <div
        ref={ref}
        className={cn('will-change-transform flex', orientation === 'horizontal' ? '' : '-mt-4 flex-col', className)}
        {...props}
      />
    </div>
  );
...
// Updated CarouselItem component to accept a renderItem prop
type CarouselItemProps = React.HTMLAttributes<HTMLDivElement> & {
  index: number;
  renderItem: (isActive: boolean) => React.ReactNode;
};

const CarouselItem = React.forwardRef<HTMLDivElement, CarouselItemProps>(({ className, index, renderItem, ...props }, ref) => {
